...
from util import show, log, parameter, gen_dct2,normalize
...
class ModelStage(nn.Module):
    ...
    def forward(self, *inputs):
        ...
        for i in reversed(range(self.depth - 1)):
            ...
            a3 = self.act(t[0], self.actwi)
            w.add_histogram('b11', x, 0)
            w.add_histogram('b12', a3, 0)
            for ii in range(3):
                w.add_image('b11'+str(ii),normalize(x[0,ii].unsqueeze(0)),0)
                w.add_image('b12'+str(ii),normalize(a3[0,ii].unsqueeze(0)),0)
