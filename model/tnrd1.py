...
from util import kaiming_normal, parameter, normalize, sleep
...
class Stage(nn.Module):
    ...
    def forward(self, *inputs):
        ...
        for i in range(1):
            ...
            t.append(x)
            x = self.rbf(x, self.actw[0])
        x = F.conv2d(self.pad(x), f[1], self.bias[1])
        t.append(x)
        x = self.rbfg(x, self.actw[0])
        for i in reversed(range(self.depth)):
            if i != self.depth - 1:
                x *= self.rbfg(t[i], self.actw[0])
            x = self.crop(F.conv_transpose2d(x, f[i]))
        eta = o.eta
        x = (xx - eta * (x + self.lam.exp() * (xx - y))) / o.ioscale
        w.add_image("eta=" + f'{eta:02}', normalize(x[0]), 0)
        # sleep(3)
        return x
