...
# >0 pow 2
class ELUP(nn.Module):
    def __init__(self):
        super(ELUP, self).__init__()
        self.zero = torch.tensor(0).float().to(o.device)
        self.half = torch.tensor(1 / 2).float().to(o.device)
        self.quarter = self.half / 2

    def forward(self, i):
        # return torch.max(i, self.zero) + torch.min(i.exp() - 1, self.zero)
        # return F.relu(i).pow(2)+ torch.min(i.exp() - 1, self.zero)
        # return F.relu(i)
        return (
            torch.min(F.elu(i), self.half)
            + torch.max(F.relu(i).pow(2), self.quarter)
            - self.quarter
        )
