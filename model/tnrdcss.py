...
class Stage(nn.Module):
    def __init__(self, stage=1):
        ...
        actw2 = [torch.randn(1, o.channel, o.penalty_num[i], 1, 1) for i in range(depth-1)]
        filter = [
        ...
        # bias2 = [torch.randn(o.channel) for i in range(depth)]
        self.lam = lam = parameter(lam)
        self.bias = bias = parameter(bias, o.bias_scale)
        # self.bias2 = bias2 = parameter(bias2, o.bias_scale)
        self.filter = filter = parameter(filter, o.filter_scale)
        self.filter2 = filter2 = parameter(filter2, o.filter_scale)
        self.actw = actw = parameter(actw, o.actw_scale)
        self.actw2 = actw2 = parameter(actw2, o.actw_scale)
        ...
        convt = [
            nn.ConvTranspose2d(o.channel, 1, o.filter_size[0]),
            *(
                nn.ConvTranspose2d(o.channel, o.channel, o.filter_size[i])
                for i in range(1, depth)
            ),
        ]
        rbf = [
        ...
        for i in range(depth):
            ...
            conv[i].bias = bias[i]
            convt[i].weight = filter2[i]
            # convt[i].bias = bias2[i]
            rbf[i].w = actw[i]
            if i!=depth-1:
                rbf2[i].w = actw2[i]
        ...
        rcc = lambda i: (rbf2[i], convt[i], crop[i])
        m = [
            None,
